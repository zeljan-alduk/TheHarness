...
pub async fn run_task(cfg: &Config, client: &Client, task_dir: &Path, spec: &TaskSpec, verbose: bool) -> TaskResult {
    ...
    let mut ctx = ToolCtx {
        ...
        subagent: None,
        redact_secrets: cfg.security.redact_secrets, hooks: cfg.hooks.clone(), todos: Default::default(), lsp_servers: cfg.lsp.servers.clone(), format: cfg.format.clone(), extra_roots: vec![], approver: None, inbox: Default::default(), cancel: None, cwd: None, session_id: None,
    };
