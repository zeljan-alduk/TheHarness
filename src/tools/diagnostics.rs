...
impl Tool for Diagnostics {
    ...
    async fn call(&self, args: Value, ctx: &ToolCtx) -> Result<ToolOutput> {
        ...
        // Python
        let py_target = if ext == "py" { shq(&target.display().to_string()) } else { ".".into() };
        ...
        if has("tsconfig.json") && (target.is_dir() || ext == "ts" || ext == "tsx") {
            ...
            let o = run(format!("node --check {} 2>&1 | head -30", shq(&target.display().to_string()))).await?;
        ...
        // Shell
        if ext == "sh" { let o = run(format!("sh -n {} 2>&1", shq(&target.display().to_string()))).await?; sections.push(format!("sh -n: {}", if o.success() { "ok".to_string() } else { o.stderr.trim().to_string() })); }
